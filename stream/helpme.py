...
parser = argparse.ArgumentParser(description="Lance le serveur avec un numéro de port spécifique")
...
class SongFiles():
    ...
    def get_new_id(self) -> int:
        ...
        """
        id = 0
        dossiers = os.listdir(self.dossier_musiques)
        for dossier in dossiers:
            fichiers = os.listdir(os.path.join(self.dossier_musiques, dossier))
            for fichier in fichiers:
                id_fichier, titre, auteur_and_ext = fichier.split("-")
                id = max(id, int(id_fichier))
        return id + 1
...
class PlayerManager():
    ...
    def playSong(self, title, current=None) -> int:
        ...
        song = self.searchWithText(title)[0] # Récupérer la musique
    ...
    def stopSong(self, port, current=None) -> bool:
...
class MusicLibraryI(MusicLibrary):
    def __init__(self):
        self.playerManager = PlayerManager()
    
    def runCommand(self, command, current=None) -> int:
        # commande peux être égal à ["jouer", "titre"]
        # ou égale à ["stop", "port"]
        print("Commande reçue : ", command)
        if len(command) == 2:
            if command[0] == "jouer":
                return self.playerManager.playSong(command[1])
            elif command[0] == "stop":
                return self.playerManager.stopSong(int(command[1]))
